...
public class TaskDetailsLayout extends AbstractMasterDataBaseLayout {
    ...
    public void onAttach(AttachEvent attachEvent) {
        ...
        createButtons();
        comboBoxTeam = createComboBox("TaskDetailsLayout.field.team.label",
                model.getTeam(),
                serviceProvider.getTeamService().findAll().toArray(new Team[0]));
        comboBoxTeam.setItemLabelGenerator(team -> comboBoxItemLabelGenerator != null
                ? comboBoxItemLabelGenerator.getLabel(Task.TEAM, team)
                : "" + team.getTitle());
        comboBoxTeam.setClearButtonVisible(true);
        comboBoxProject = createComboBox("TaskDetailsLayout.field.project.label",
                model.getProject(),
                serviceProvider.getProjectService().findAll().toArray(new Project[0]));
        comboBoxProject.setItemLabelGenerator(project -> comboBoxItemLabelGenerator != null
        ...
        comboBoxTaskStatus = createComboBox("TaskDetailsLayout.field.taskstatus.label",
                model.getTaskStatus(),
                TaskStatus.values(),
                componentFactory.getTaskStatusItemLabelGenerator());
